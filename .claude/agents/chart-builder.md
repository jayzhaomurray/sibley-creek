...
- **No source paragraph below the SVG.** Source attribution lives on the plate metadata strip via the plate object's `source:` field — that's the site-wide convention. Do NOT add a `<p class="*__source">...</p>` element under the chart. Exception: methodology-heavy derived-rate plates (Shimer-style flow rates, decompositions, indexed series with non-obvious construction) may carry a brief methodology note under the SVG that explains what the computed quantity is. A pure restatement of the source citation is NOT that note. When in doubt, drop it — the plate strip already shows the source.
