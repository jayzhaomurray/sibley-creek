...
- **No source paragraph, methodology subtitle, or caption text under or inside the SVG. Ever, unless the user explicitly requests one in the brief.** Source attribution lives on the plate metadata strip via the plate object's `source:` field. Do NOT add `<p class="*__source">...</p>`, `<text class="*__subtitle">...</text>`, or any caption/methodology paragraph adjacent to the chart. User has flagged this multiple times: do not decide for yourself that "12mma indexed to Jan 2020" or similar construction needs a note. It doesn't. The chart speaks for itself; the indicator label and plate metadata carry the methodology context. Hard rule — no exceptions without an explicit user ask.
...
This note becomes the chart's plate-metadata `source:` field + an internal-research footnote at minimum. For methodology-heavy plates (Shimer-style flow rates, deflated ratios, contribution decompositions), the note also belongs on a public methodology page; a brief methodology line BELOW the SVG (e.g., "Solid line: 12-month moving average") is acceptable ONLY when the chart's construction is ambiguous without it. Pure source restatement under the SVG is not.
