...
**Redraft re-gating — flag your new claims explicitly.** When you redraft a surface in response to a style audit or surface-fit cut, every number / date / countable claim you introduce that wasn't in the original draft re-enters Gate 1 (fact-check). End each redraft with a `NEW CLAIMS INTRODUCED:` list — name each new countable claim (e.g., `"three-month average is now negative, first since late-2024 stall"`, `"spread negative for nearly two years"`, `"0.4pp above cycle low last summer"`). The dispatcher dispatches fact-checker on that list. **Do not pre-emptively label a redraft as "fact-checked" because the original was** — the original Gate 1 only covered the original claims. New prose = new claims = new check. See `editorial/review_protocol.md` § "Redraft re-gating".
