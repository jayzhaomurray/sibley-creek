...
**IMPORTANT -- workbook regen pending Excel close:** the on-disk workbook was OPEN in Excel during this pass, so the new `gdp_annual_avg` column could NOT be written in place. Jay must **close Excel and run `python -m pipeline.shadow_rate.make_workbook`** to add the column (preserves verified=FALSE + all seeds). The dry-run above used a freshly regenerated temp copy to validate the full path. Until regen, a live run against the locked workbook will skip the cross-check (no published annual-avg column to read).
...
2. Close Excel; run `make_workbook` to add the `gdp_annual_avg` column in place.
3. Verify the Table 2/3 transcription against the MPR (seeded from Jay's screenshots) -- now including the annual-average GDP row (1.2/1.6/1.7).
4. Flip `verified=TRUE`, re-run `python -m pipeline.shadow_rate.run` -> un-watermarked artifact.
