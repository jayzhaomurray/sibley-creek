...
**Last updated:** 2026-07-20 (June CPI commentary published; site-wide staleness audit; build-monthly CI outage found + data manually refreshed)
...
- **Local checkout hygiene fixed**: main checkout was 26 commits behind origin with ~369 stale staged files left over from the unresolved-merge situation mentioned in the (now-lost) July 15 notes. Verified the staged content was strictly older/inferior to origin (confirmed via diff, e.g. a CPI meta file's staged fetch timestamp was July 7 vs origin's already-newer July 10), so reset to origin/master. This is also what wiped the uncommitted STATUS.md content above - see note.
- **Found: `build-monthly.yml` (build-data-daily) has failed every scheduled weekday run since ~2026-06-22** - a full month - always at the `python -m pipeline.build` step, starving CPI/GDP/labour/housing/trade/fiscal of fresh data. Confirmed via GitHub API run history (couldn't pull raw CI logs - no admin-scoped token, no `gh` CLI). Ran the exact same code locally at origin/master tip and it succeeded cleanly, pulling genuine new data (June CPI, June housing/labour, July monetary) - so this is CI-environment-specific, not a code bug. Leading hypothesis: `pipeline/requirements.txt` has no lockfile and uses loose `>=` ranges, so a fresh CI pip install can silently resolve a different (breaking) package version than what's already installed locally. **backend-engineer dispatched to confirm root cause and fix durably** (pending).
- **Manually pushed the refreshed data** (master `8cc93fa`, deploy green, live-verified `/inflation/` now stamped "CPI, Jun 2026"): unblocks the site immediately regardless of the CI fix timeline.
- **Found + dispatched fix for a second bug**: `/inflation/` section's hand-authored lede + interpretation prose was describing the OLD May-vs-April story ("climbed to 3.2%...") even after the chart data updated to June - inflation isn't on the mechanical-prose system (only markets is), so the blurb didn't auto-correct. **writer agent dispatched** to redraft grounded in the verified June numbers above; still needs to clear fact-check + style-editor + surface-fit gates before it's live. Checked labour/housing/output/monetary - all already correctly describe their own current vintage, no other section affected.
- **CI root cause found + fixed (master `c23b988`):** `pipeline/fetch/cpi_components.py` had `MAPPING_PATH` hardcoded to an absolute path on Jay's own machine (`C:/Users/jayzh/Documents/boc-tracker/...`). Worked locally, `FileNotFoundError` on every CI runner - this single fetcher failure has been failing the whole fail-loud `pipeline.build` and blocking every scheduled data refresh since **2026-05-20** (two months, not the ~1 month I estimated from run history alone). Fixed: vendored the mapping into `pipeline/catalog/cpi_breadth_mapping.json`, path now resolves relative to the module file, added a regression test (`pipeline/tests/test_cpi_components.py`) that fails fast if this ever regresses, and wired a self-resolving GitHub Issue alert into `build-monthly.yml` (mirrors the existing `deploy.yml` pattern) so a recurrence is loud instead of silent for two months. Verified locally end-to-end (313 tests pass, pipeline completes clean) but could NOT force a live re-run to confirm in real CI (token lacks `actions:write` for `workflow_dispatch`) - next scheduled run is tomorrow 14:00 UTC; the new Issue alert will fire if it's still broken.
- **Inflation prose redraft: fact-check done (2 rounds), all gates green, style-editor dispatched.** Round 1 verified plates 1-4 + header/tile clean against the refreshed pipeline data. Round 2 caught plate-5 citing stale Q1 2026 expectations data ("expectations never moved") when Q2 2026 was already in `data/raw/*` (BoC officially released it 2026-07-06, confirmed via bankofcanada.ca - not a vintage-ambiguity call, just wrong); writer rewrote it, but the rewrite introduced an anachronism (attributed the Q2 survey jump to the July ceasefire collapse, which happened 3+ weeks AFTER the surveys closed in May) - caught by a second fact-check pass, fixed directly (mechanical correction, already sourced): the real driver was oil hitting ~$101/bbl *during* the May field period, not the later ceasefire. Also hit and fixed three build-gate issues myself: a pre-existing uncovered "1%" citation token on plate-3, an invalid `commentary:*` source-type (only `pipeline:*`/`card:*`/`derived` are allowed - fixed to `derived`), and a length-budget overrun on plate-5 (128W -> 105W, hard cap 110W). **Near-miss, caught before push:** I initially deleted the stale Q1 BOS/CSCE `_pending` cards as "superseded" - they're actually still live citations for the unrelated `boc-fed-divergence` research dive (14 references). Restored via `git checkout`, registered a proper new card (`boc_q2_2026_bos_csce_release`) instead of touching the old ones. Full build green (`npm run build`, all gates pass). **Style-editor dispatched** (voice/concision pass); surface-fit is the last gate after that, then ship. **Not yet committed - still uncommitted in working tree.**
- **Style-editor pass done, editorial-director (surface-fit, final gate) dispatched.** Style-editor tightened all 5 plates + header/tile for concision (all now within length budgets), fixed a two-clause title violation on plate-4 ("undoing May's acceleration, but services are picking up speed" -> "Energy's collapse is masking a pickup in services inflation" - a real emphasis change, escalated rather than decided solo) and flagged plate-5 as citation-dense (90W, over soft target but under hard cap, carrying 10 verified claims). I caught one more mechanical break myself (a citation phrase case-mismatch from the style edit) before it reached the gate. Editorial-director now deciding both escalations + doing the normal fresh surface-fit pass; will give a go/no-go on shipping.
- **CI fix CONFIRMED live (2026-07-20 15:47 UTC):** the next scheduled `build-data-daily` run after the fix landed completed successfully (`event: schedule`, not a manual dispatch - confirmed via GitHub API), first green run since 2026-05-20. Two months of continuous failure, closed. Data auto-refreshed and auto-pushed by the bot (`15ae99e`/`835b53d`), merged into master alongside this file.
- **Follow-ups:** (1) ship the inflation redraft once surface-fit clears; (2) above is the recovered June-July narrative - see the note at the top on completeness.

---

### DECISION DAY 2026-07-15 - BoC held at 2.25% (sixth straight); commentary + monetary update live
- **Commentary published** (master `5c4d140` + slug fix `2622223`, deploy green, live-verified 200): `https://sibleycreek.ca/research/commentaries/boc-2026-07-15/` (Jay's PDF "Bank of Canada stays on hold at 2.25%"; take: bank upbeat, more comfortable with its stance than June; both main risks receded per Jay's read). Newest entry, leads the splash. Gotcha hit: `section: "policy"` is not a valid SectionSlug (it's `"monetary"`) - first deploy failed astro check on exactly this; fixed and redeployed.
- **/monetary/ + splash refreshed to the July decision** (master `609128e`, full 3-gate loop passed, deploy watched at push): sixth consecutive hold; statement calls the rate "appropriate" (June's "dilemma" framing retired - fact-check pinned "dilemma" provenance to Macklem's June OPENING STATEMENT, not the press release); hold-count card enumerates 6 FADs, next FAD Sep 2, 2026; market values to Jul 13 close (2y 2.88%, spread -138 bps). Fact-check REJECTED "both risks have eased since June" - July communications say the Middle East conflict RE-ESCALATED in recent days (diverges from the commentary's "both receded" framing; site copy follows the primary source, Jay's PDF is his signed take). Blurb close reframed per surface-fit gate: "Markets see it the same way..." (old "moved the other way" misread as post-decision reaction).
- **Follow-ups (as of 2026-07-15, likely superseded by now):** (1) plate-3's "implied path edges higher into late 2026" citation still rests on June-10 CORRA-futures analysis - needs a fresh futures read; (2) July statement source card not yet created (verbatim quotes cited derived-with-excerpt, same fast-follow as June's); (3) main checkout's unresolved-merge state - RESOLVED 2026-07-20 (see today's entry above).

### DEPLOY UNFROZEN + FISCAL IN CAROUSEL - fixed 2026-06-18 (master `daad8a7`, deploy green, live-verified)
The fiscal chartbook shipped panels 6-11 in `fiscal.astro` but never added their specs to `pipeline/io/panel_data.py` (only 1-5 were declared). Publish baked 6-11 into `fiscal.json` statically; the 2026-06-17 daily refresh regenerated from the incomplete spec, dropped 6-11, failed `check_panel_data_wired`, and froze ALL deploys - so the carousel fix (`3015272`, which drops fiscal from the splash-carousel filter) sat on master undeployed and fiscal never appeared in the homepage carousel. Fixed durably by porting the panel-6..11 spec block into master's `panel_data.py` (they read already-tracked `frt_*` derived CSVs; `_read_slot` reads them directly, no fetcher needed). Regenerated `fiscal.json` (panels reproduce identically: 48/48/48/48/8/6 records), built green locally, pushed. Deploy `daad8a7` succeeded; live homepage now carries all 8 carousel slides incl. `data-slug="fiscal"`. Lesson -> memory `feedback_new_panels_need_spec_or_refresh_wipes`. **Then (also 2026-06-18, master `b7c86b4`, deploy green, live-verified):** the `/overview/` indicator grid still hard-coded a fiscal "In development" placeholder cell - replaced it with the normal live `SectionPanel` (renders Federal balance FYTD -$55.3B). Also dropped the stale `debt-service-share` print scaffold from the fiscal canon in sections.ts (pipeline stopped emitting it 2026-05-28, so it rendered as TK cells and tripped the reader-copy-leakage gate). **Note:** the `macro-fiscal` worktree branch had diverged far beyond fiscal (recession-watch, prose-engine deletions, markets/monetary rewrites) - do NOT merge it wholesale; the fiscal page itself already shipped via `25874b0`.

**Phase 3 - /overview/ fiscal indicator panel reworked (2026-06-18, master `2af7fcc`, deploy green, live-verified).** Per Jay: (1) renamed "Federal balance (FYTD)" -> "Federal budget balance" in both render spots; (2) sparkline changed line -> bars via `tileChartKind: "bars"` in sections.ts; (3) rewrote the tileLine to describe ONLY the sparkline's realized data ("The federal deficit is running wider this fiscal year than last."), citing card `dof_fiscal_monitor_debt_service_share`; (4) added three actuals-only supporting prints reading tracked `frt_*` derived CSVs - Federal debt 41.2% of GDP (-0.9pp), Program expenses 16.1% of GDP (-0.1pp), Interest 10.5% of revenue (+0.2pp), all FY2024-25. New `is_forecast` filter in `_read_series` (site_data.py) drops projection rows so tile value/delta/sparkline are realized-only; new `fiscal-year` as-of format ("FY 2024-25") and `fy-yoy` delta window added. **Freshness catch:** first pass used the annual FRT series (latest FY2024-25, -$36.3B) - Jay flagged "don't we have another year of data already?"; reverted the headline to the monthly Fiscal Monitor (`federal_budget_ytd`, FY2025-26 through Mar 2026, -$55.3B; delta -$12.1B YoY vs prior-year pace). Reinforces `feedback_always_freshest_vintage`.
...
- **/monetary/ refreshed to the June decision** (master `9a702ae`, deploy green). Take-driven refresh. April's mild easing lean -> the Bank's explicit two-sided "dilemma" framing (weak growth + inflation back toward 3%); fifth consecutive hold; plates 1-3 + lede + tile rewritten, plates 4-5 to the June-3 balance-sheet vintage. Backend also fixed a real slug-drift bug (`overnight_rate_target`->`overnight_rate`) that had silently pinned the monetary tile/spread to April vintage since May 11. Hold-count card -> 5 (next FAD July 15, not 29).
- **BoC decision-day live monitor BUILT** (master, `pipeline/tools/boc_live_monitor.py`): one-command local dashboard (`.venv\Scripts\python.exe -m pipeline.tools.boc_live_monitor` -> 127.0.0.1:8787) - CORRA-implied path + USDCAD + CGB/CGZ GoC bond futures + TSX/WTI, auto-refresh, "freeze baseline" button.

### MARKETS PROSE IS NOW MECHANICAL - shipped to master 2026-06-10 (merge `15a8f75`, deploy green, live-verified)
The /markets/ page text is now a deterministic FUNCTION OF THE DATA - no LLM and no human in the daily loop. Root problem fixed: prose went 4 weeks stale and FALSE ("oil still over $100" while the chart showed $92) because hand-written text can't track daily data. Now every figure renders from `data/site/panel_data/markets.json` at build time (slot-interpolated), and every qualitative claim is a predicate-gated variant - the build picks the variant whose predicate is TRUE; a sentence with no true variant silently drops. Shipping a false claim is structurally impossible. Architecture: renderer `src/lib/prose/` (`renderSectionProse("markets")`), templates `editorial/prose_templates/markets.yaml`, render-dump tool `node scripts/render_prose.mjs markets`. **Follow-up surfaced (2026-06-10, likely still open):** monetary section is the next mechanical-prose cutover candidate, same playbook - relevant now given today's finding that inflation's hand-authored prose just went stale too; worth raising with Jay whether inflation should get the same cutover.

### Labour section refreshed to May 2026 LFS - pushed 2026-06-05 (master `a3e7370`)
All 8 labour surfaces rewritten - the May print INVERTED the prior "loosening on the intensive margin" thesis (UR 6.6%, +87.8k, hours Y/Y flipped positive). New Tier-A card `statcan_daily_lfs_2026-05`.

### Fiscal chartbook - branch `fiscal-chartbook` - SHELVED as of 2026-06-10, status since unknown
Jay's call: "i think the charts still needed work" - not publish-ready, do NOT re-pitch publishing as a quick flip. Branch was ~67 commits behind master as of 2026-06-10. Live `master` serves the `/fiscal/` coming-soon placeholder as of that date (may have changed since - the fiscal panel data IS live on /overview/ and in the carousel per the entries above, so at minimum the indicator-panel work shipped even if the dedicated /fiscal/ page didn't).
