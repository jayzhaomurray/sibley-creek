...
- **Inflation prose redraft done, fact-check gate dispatched (in progress):** writer rewrote heroKicker/tileLine/blurb in `sections.ts` + all 5 plates in `inflation.astro` for June's actual numbers (2.8% headline, cores 1.8%/1.9%, breadth reversal, energy/services rotation). Three claims flagged `source: "derived"` pending card registration (ceasefire-gasoline link, World Cup tourism prices, $14/barrel oil move) - fact-checker dispatched to verify + card them. Still needs style-editor + surface-fit gates after that before it ships. **Not yet committed or pushed.**
