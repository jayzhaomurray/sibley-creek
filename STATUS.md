...
**Artifacts:** package `pipeline/shadow_rate/` (28 tests green); punch-in workbook `work/research/shadow_rate/boc_shadow_inputs_2026Q2.xlsx` (now 4 sheets: an `output` sheet — full per-quarter calc decomposition, placed first — is written back on every run alongside the 3 input sheets; source_ref provenance, `verified=FALSE`); chart `work/research/shadow_rate/boc_shadow_path_2026-04.html` (UNVERIFIED watermark); methodology `claude-ref/research/shadow_rate/shadow_rate_methodology_2026-04.md`; output `data/processed/boc_shadow_rate.csv` + sidecar.
